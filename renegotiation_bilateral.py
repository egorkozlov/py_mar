...
def ren_bilateral_wrap(setup,vy,vfy,vmy,vfn,vmn,vf_all_s,vm_all_s,aleft_c,                       
    ...
    #Before renegotiation, check more     
    vfn1=setup.vsgrid_s2.apply_preserve_shape(vmn) #(setup.vsgrid_s.apply_preserve_shape((vfn*(-0.5))**(-2)))**(-0.5)/(-0.5)#
    vmn1=setup.vsgrid_s2.apply_preserve_shape(vfn)#(setup.vsgrid_s.apply_preserve_shape((vmn*(-0.5))**(-2)))**(-0.5)/(-0.5)#
    ...
    if np.all(abs(np.diff(setup.agrid_s)[0]-np.diff(setup.agrid_s))<1e-6):
        ...
        vout, vfout, vmout, thetaout, yes, ithetaout, bribe, iaout_f, iaout_m = \
            ren_loop_bilateral1(vy,vfy,vmy,vfn,vmn,
                               vf_all_s,vm_all_s,aleft_c,
                               ia_div_fem,ia_div_mal,
                               setup.agrid_s,#*0.753853,
                               tgrid,vmn1,vfn1,setup.isgrid,setup.vsgrid_s2.i,setup.sgrid_s2)
                ...
                if sf_i >= 0 and sm_i >= 0:
                    ...
                    if sf_i < 0 and sm_i < 0:                        
                        ...
                        if sm_i > 0 and sf_i < 0:
                            ...
                            for i in range(1,ia_m_def-1):
                                sf_i_new  = vfy[ia,ie,it] - vfn_as1[min(ia_f_def-i,na_s-1),ie]
                                sm_i_new  = vmy[ia,ie,it] - vmn_as1[min(ia_m_def+i,na_s-1),ie]
                                if sf_i_new < 0 and sm_i_new < 0:                              
                                    do_divorce = True
                                    ia_f_new=ia_f_def+i
                                    ia_m_new=ia_m_def-i
...
@njit
def ren_loop_bilateral2(vy,vfy,vmy,vfn,vmn,vfn_as,vmn_as,aleft_c,ia_f_def_s,ia_m_def_s,agrid_s,thtgrid):
    #print('bilateral hi!')
                ...
                if sf_i >= 0 and sm_i >= 0:
                    ...
                    if sf_i < 0 and sm_i < 0:                        
                        ...
                        if sm_i > 0 and sf_i < 0:
                            ...
                            for i in range(1,ia_m_def-1):
                                sf_i_new  = vfy[ia,ie,it] - vfn_as[min(ia_f_def-i,na_s-1),ie]
                                sm_i_new  = vmy[ia,ie,it] - vmn_as[min(ia_m_def+i,na_s-1),ie]
                                if sf_i_new < 0 and sm_i_new < 0: 
                                    #print(11111,i)
                                    do_divorce = True
