        ...
        for j in range(len(agrid)):
            for h in range(len(setup.thetagrid)):
                ...
                if not np.all(np.diff(V_couple[j,(i*npsi):(npsi+i*npsi),h],axis=0)>0):
                    print(333,uf[j,(i*npsi):(npsi+i*npsi),h]+psi_r[j,(i*npsi):(npsi+i*npsi),h])
                    print(444,c_opt[j,(i*npsi):(npsi+i*npsi),h])
                    print(111,i,(npsi+i*npsi),222,j,h,333,np.take_along_axis(EVf_all,i_opt,0)[j,(i*npsi):(npsi+i*npsi),h])
                    print(222,i,(npsi+i*npsi),222,j,h,333,np.take_along_axis(EVm_all,i_opt,0)[j,(i*npsi):(npsi+i*npsi),h]) 
