...
from rw_approximations import rouw_st,rouw_nonst, normcdf_tr
...
class ModelSetup(object):
    def __init__(self,nogrid=False,divorce_costs='Default',separation_costs='Default',**kwargs): 
        ...
        p['A'] = 0.1#1.0 # consumption in couple: c = (1/A)*[c_f^(1+rho) + c_m^(1+rho)]^(1/(1+rho))
        p['crra_power'] = 1.5
        p['couple_rts'] = 0.0 
        p['sig_partner_a'] = 0.1
        p['sig_partner_z'] = 1.1#0.4
       ...
       # p['f_wage_trend'] = [0.3 for t in range(T)]
        ...
        self.agrid_c[-2]=26        # this builds finer grid for potential savings
        ...
        tune_s=1.0
        self.mgrid = np.geomspace(mmin+tune_s,mmax+tune_s,num=1200)-tune_s
        self.u_precompute()
    ...
    def mar_mats_iexo(self,t,female=True,trim_lvl=0.001):
        ...
        for iz in range(n_zown):
            ...
            if female:
                p_zm  = int_prob(z_partner, mu=z_own[iz]+1,sig=sig_z_partner)
                p_zf  = zmat_own[iz,:]
            else:
                p_zf  = int_prob(z_partner, mu=z_own[iz]-1,sig=sig_z_partner)
                p_zm  = zmat_own[iz,:]
            #sm = sf
