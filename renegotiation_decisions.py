...
def v_ren_vt(setup,V,marriage,t,return_extra=False,return_vdiv_only=False,rescale=True,
    ...
    def v_rescale(v,it_out):
        ...
        vo = v.copy()
        if vo.ndim>3:
            itheta_in = np.broadcast_to(np.arange(thtgrid.size,dtype=np.int16)[None,None,:,None],it_out.shape)
        else:
...
def v_div_vartheta(setup,dc,t,sc,Vmale,Vfemale,izf,izm,
    ...
    share_fem, share_mal = dc.shares_if_split_theta(setup,setup.ashare,dc)
    #share_fem, share_mal = fun(setup.ashare)
