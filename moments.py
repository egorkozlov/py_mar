...
def moment(mdl_list,agents,agents_male,draw=False,validation=False,nocensor=False,transform=False):       
...
#matrixes coming from simulations       
    ...
    setup = mdl.setup      
    female=np.array([agents.is_female],dtype=bool)[0,...]
    cons=agents.c
    
    cons[(female) & (state>1)]\
        =cons[(female) & (state>1)]*\
            setup.c_mult(theta_t[(female) & (state>1)])[0]
    cons[~(female) & (state>1)]=\
        cons[~(female) & (state>1)]*\
            setup.c_mult(theta_t[~(female) & (state>1)])[1]
    ...
    if nocensor:keep=np.ones(keep.shape,dtype=bool)
    state=state[keep,]        
    ...
    is_iexo= -1*np.ones((N,nspells,mdl.setup.pars['T']),dtype=np.float32) 
    ...
    for t in range(1,mdl.setup.pars['T']):       
        ...
        if draw: 
            ...
            is_iexo[((~ichange)   & (censored[:,t]==True)),ispell[((~ichange) & (censored[:,t]==True))], 
             sp_length[((~ichange) & (censored[:,t]==True)),ispell[((~ichange) & (censored[:,t]==True))]]-1]=\
              iexo[((~ichange)  & (censored[:,t]==True)),t] 
        ...
        if draw:  
            is_iexo[ichange,ispell[ichange]+1,0]=iexo[ichange,t] 
    ...
    if draw: 
        allspells_iexo=is_iexo[is_spell] 
    ...
    if draw:   
        ...
        if np.var(data_rel_panda['uni'])>0.0001:    
            ...
            #beta_dur_s=FE_ols.params['uni']       
                ...
                ####################################################################
                #  Marriage - RISK SHARING and Divorce by partnership
                #  nocensor=True
                #  For controlling selection in, set selin=True, selinm=True in sim.
                #  For controlling for selection out, set selout=True in sim.
                #  Also, below set data_mar_panda['mcsw']==1 if control for selection,
                #  otherwise data_mar_panda['mcsw']==0
                #  in residuals set N=140000
                ##############################################################
                 
                #Analysis of divorce 
            try: 
                #Reshape data relationship-years
                ...
                data_mar_panda['age']=data_mar_panda['age']+data_mar_panda['t']-1 
                ...
                data_mar_panda['thi']=allspells_th[atype==2][allspells_asse[atype==2]>-1]
                data_mar_panda['psi']=setup.all_indices(0,allspells_iexo[atype==2][allspells_asse[atype==2]>-1])[-1]
                 
                #######################################
                #Analysis on married couples and assets
                #######################################
                ...
                data_mar_panda=data_mar_panda.loc[(data_mar_panda['mcsw']==0)] 
                 
                ###########################
                #Analysis of risk Sharing 
                #########################
                data_mar_panda['dcons']=data_mar_panda['cons'].diff() 
                data_mar_panda['dwf']=data_mar_panda['wf'].diff() 
                data_mar_panda['dwm']=data_mar_panda['wm'].diff() 
                ...
                time=(data_mar_panda['age']<=45)
                time2=(data_mar_panda['age']<=30)
                dur=(data_mar_panda['t']==1) 
                durp=(data_mar_panda['t']!=1) 
                rell=(data_mar_panda['rel']==1)
                
                 
                #Regressions
                rsw_mut=smf.ols(formula='dcons~ dwf+C(age)', data =data_mar_panda 
                               [~uni & fem & els & time & durp & rell]).fit() 
                ...
                ################################
                #Relationship types and split
                ################################
                uni_surved=np.mean(data_mar_panda[uni  & els & time2 & dur & rell]['duration']>=15) 
                mut_surved=np.mean(data_mar_panda[~uni & els & time2 & dur & rell]['duration']>=15) 
                print('Share marriages surviving more than 51yrs is mut {} uni {}'.format(mut_surved,uni_surved)) 
            ...
            try: 
                #################################################################
                #  COHABITATION - RISK SHARING , BREAKUP and money at divorce/breakup
                #  nocensor=True, transform=True for split experiment  only.
                #  For controlling selection in, set selin=True, selinm=False in sim.
                #  For controlling for selection out, set selout=True in sim.
                #  Also, below set data_coh_pandac['mcsw']==1 if control for selection,
                #  otherwise data_coh_pandac['mcsw']==0
                #  in residuals set N=140000
                ##############################################################
                ...
                #Variable
                data_coh_pandac['age']=data_coh_pandac['age']+data_coh_pandac['t']-1 
                ...
                data_coh_pandac['csw']=allspells_csw[atype==3][allspells_wm[atype==3]>-1] 
                data_coh_pandac['mcsw']=data_coh_pandac.groupby(['index1'], sort=False)['csw'].max() 
                data_coh_pandac['psi']=setup.all_indices(0,allspells_iexo[atype==3][allspells_asse[atype==3]>-1])[-1]
                data_coh_pandac['thi']=allspells_th[atype==3][allspells_asse[atype==3]>-1]
                data_coh_pandac['diff']=0.0 
                data_coh_pandac.loc[(data_coh_pandac['thi']==data_coh_pandac['th']),'diff']=1.0 
                 
                #Keep selection/no selection
                data_coh_pandac=data_coh_pandac.loc[(data_coh_pandac['mcsw']==0)] 
                 
                #Generate first differences
                data_coh_pandac['dcons']=data_coh_pandac['cons'].diff()
                data_coh_pandac['dwf']=data_coh_pandac['wf'].diff() 
                data_coh_pandac['dwm']=data_coh_pandac['wm'].diff() 
                ...
                time=(data_coh_pandac['age']<=45)
                time2=(data_coh_pandac['age']<=30)
                dur=(data_coh_pandac['t']==1) 
                durp=(data_coh_pandac['t']>1) 
                rell=(data_coh_pandac['rel']==1)
                 
                #Regressions and results below 
                unif=smf.ols(formula='dcons~ dwf+C(age)', \
                               data =data_coh_pandac[uni & els & fem & time & durp & rell]).fit() 
                unim=smf.ols(formula='dcons~ dwm+C(age)', \
                               data =data_coh_pandac[els & ~fem & time & durp & rell]).fit() 
                ...
                #Relationship types and split
                ################################
                uni_surved=np.mean(data_coh_pandac[uni  & els & time2 & dur & rell]['duration']>=15) 
                mut_surved=np.mean(data_coh_pandac[~uni  & els & time2 & dur & rell]['duration']>=15) 
                print('Share not breking up more than 15yrs is mut {} uni {}'.format(mut_surved,uni_surved)) 
