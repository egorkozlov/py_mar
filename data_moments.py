...
def compute(hi,d_hrs,d_divo,period=3,transform=1):    
    ...
    reg=smf.ols(formula='wls~ C(state)+C(year)+C(age)+C(mar)', data = d_hrs2[(d_hrs2['age']<=38) & (d_hrs2['age']>=23)]).fit()    
    fls_ratio=-reg.params['C(mar)[T.1.0]']
...
def dat_moments(sampling_number=5,weighting=True,covariances=False,relative=False,period=3,transform=1):    
    ...
    if weighting:    
        ...
        #Compute optimal Weighting Matrix    
        col=np.concatenate((hazm,hazs,hazd,emar,ecoh,fls_ratio*np.ones(1),wage_ratio*np.ones(1),beta_unid*np.ones(1),mean_fls*np.ones(1)),axis=0)        
        dim=len(col)    
        W=np.zeros((dim,dim))    
        for i in range(dim):    
                W[i,i]=1.0/col[i]**2   
        ...
        W=np.diag(np.ones(len(hazm)+len(hazs)+len(hazd)+len(emar)+len(ecoh)+4))#two is for fls+beta_unid    
