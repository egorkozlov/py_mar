...
def mdl_resid(x=None,save_to=None,load_from=None,return_format=['distance'],
    ...
    dc = DivorceCosts(unilateral_divorce=False,assets_kept =akept,u_lost_m=ulost,u_lost_f=ulost,eq_split=0.0)
    ...
    if load_from is None:
        ...
        if not solve_transition:
            ...
            dc_before = DivorceCosts(unilateral_divorce=False,assets_kept = akept,u_lost_m=ulost,u_lost_f=ulost,eq_split=0.0)
            dc_after  = DivorceCosts(unilateral_divorce=True,assets_kept = akept,u_lost_m=ulost,u_lost_f=ulost,eq_split=0.0)
