...
def mdl_resid(x=None,save_to=None,load_from=None,return_format=['distance'],
    ...
    ulost=0
    dc = DivorceCosts(unilateral_divorce=True,assets_kept =akept,u_lost_m=ulost,u_lost_f=ulost,eq_split=1.0)
    sc = DivorceCosts(unilateral_divorce=True,assets_kept = 1.0,u_lost_m=ulost,u_lost_f=ulost,eq_split=0.0)
    ...
    W=packed_data['W']
    dat=np.concatenate((hazm_d,hazs_d,hazd_d,mar_d,coh_d,fls_d,wage_d,mean_fls_d),axis=0)
    ...
    sim=np.concatenate((hazm_s,hazs_s,hazd_s,mar_s,coh_s,fls_s,wage_s,mean_fls_s),axis=0)
