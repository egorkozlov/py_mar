...
if __name__ == '__main__':   
    ...
    def compute(data,add1,add2,add3,add4,alpha=0.05):  
        ...
        eventgrid=np.array(np.linspace(-6,6,12),dtype=np.int16) 
    ...
    #vImport the data
    datav=pd.read_stata('dive.dta')
    #datav.dropna(inplace=True)
    
    addo1=np.mean(datav.loc[(datav['event']==-1) & (datav['less1']==0),'wealth'])
    addo2=np.mean(datav.loc[(datav['event']==-1) & (datav['sex']==1),'wealth'])
    addo3=np.mean(datav.loc[(datav['event']==-1) & (datav['less1']==1),'wealth'])
    addo4=np.mean(datav.loc[(datav['event']==-1) & (datav['sex']==2),'wealth'])
    ...
    plt.savefig('eventwh_r_p.pgf', bbox_inches = 'tight',pad_inches = 0)  
    #plt.show()
    ...
    plt.xlabel('Event time (Years)', fontsize=16)    
    plt.ylabel('Net Worth (\$ 1000s)', fontsize=16)  
    plt.savefig('eventwh_r_s.pgf', bbox_inches = 'tight',pad_inches = 0)  
    #plt.show()
