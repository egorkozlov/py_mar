...
def v_mar_igrid(setup,t,V,icouple,ind_or_inds,*,female,marriage,interpolate=True,return_all=False):
   ...
   # if t==44:print('here {}'.format(np.max(abs(Vms[:,index2]-Vfs))))
    ...
    #vfout, vmout, nbsout, agree, ithetaout = mar_mat(*ins,t,setup)
    
   
    vfout, vmout, nbsout, agree, ithetaout = mar_loop(Vfm,Vmm,Vfs,Vms)
...
def v_mar_igrid_alessandra(setup,t,V,icouple,ind_or_inds,*,female,marriage,interpolate=True,return_all=False):
    # this returns value functions for couple that entered the last period with
    # (s,Z,theta) from the grid and is allowed to renegotiate them or breakup
    ...
    #cohabitation is couple based. For a given bargaining power and other
   ...
   # if t==44:print('here {}'.format(np.max(abs(Vms[:,index2]-Vfs))))
    ...
    vfout, vmout, nbsout, agree, ithetaout,i_mar = mar_loop2(Vfm,Vmm,Vfs,Vms,which)
...
@njit
def mar_loop2(vfy,vmy,vfn,vmn,which):
    ...
    for ia in range(na):
        for iexo in range(nexo):
            ...
            if good:
                ...
                # if (np.sum(abs(nbs-nbs_best)<0.0000001)>1) & (nbs_best>1e-8):
                #     wh=np.where(abs(nbs-nbs_best)<0.0000001)
                #     nbs[wh]=0.0
                #     i_best = nbs.argmax()
                #     nbs_best = nbs[i_best]
    ...
    # print('mean nash is {} '.format(np.max(abs(vfout1-vmout))))
    # assert np.max(abs(vfout1-vmout))<0.0000001
...
def mar_loop(vfy,vmy,vfn,vmn):
    ...
    for ia in range(na):
        for iexo in range(nexo):
            ...
            both = (sf_i+sm_i>0)#(sf_i > 0) & (sm_i > 0)
